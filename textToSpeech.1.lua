local textToSpeech = {}

local serpent = require "serpent"
local cmuDict = require "cmuDict"
local timings = require "timings"

local phonemesList = {"AA","AE","AH","AO","AW","AY","B","CH","D","DH","EH","ER","EY","F","G","HH","IH","IY","JH","K","L","M","N",
											"NG","OW","OY","P","R","S","SH","T","TH","UH","UW","V","W","Y","Z","ZH"}

-- associate each phoneme with a signal value (1 signal for each phoneme, though it might make sense to spell the phoneme with signals to identify it, maybe)
local signalsList = {"AA"="signal-0","AE"="signal-1","AH"="signal-2","AO"="signal-3","AW"="signal-4","AY"="signal-5","B"="signal-6",
										 "CH"="signal-7","D"="signal-8","DH"="signal-9","EH"="signal-A","ER"="signal-B","EY"="signal-C","F"="signal-D",
										 "G"="signal-E","HH"="signal-F","IH"="signal-G","IY"="signal-H","JH"="signal-I","K"="signal-1"="signal-J",
										 "L"="signal-K","M"="signal-L","N"="signal-M","NG"="signal-N","OW"="signal-O","OY"="signal-P","P"="signal-Q",
										 "R"="signal-R","S"="signal-S","SH"="signal-T","T"="signal-U","TH"="signal-V","UH"="signal-W","UW"="signal-X",
										 "V"="signal-Y","W"="signal-Z","Y"="signal-black","Z"="signal-blue","ZH"="signal-cyan"}

local entityArrangementsList = { linear = 1, block = 2, compact = 3 }

local cmuDictFileTable = {}
local timingsTable = {}

local function indexOf(table,value)
	for k,v in pairs(table) do
		if v==value then
			return k
		end
	end
	return nil
end

local function getTimerEntities(length)
	return {{entity_number=1, name="constant-combinator" ,position={x=0.0,y=0.0}, direction=4,
  control_behavior={filters={{signal={type="virtual",name="signal-0"},count=1,index=1}},is_on=false},
  connections= {["1"]={red={{entity_id=2,circuit_id=1}}}}},
	{entity_number=2,name="decider-combinator",position={x=0.0,y=1.5}, direction=4,
  control_behavior={decider_conditions={first_signal={type="virtual",name="signal-0"},constant=length,comparator="<",output_signal={type="virtual",name="signal-0"},copy_count_from_input=true},is_on=false},
  connections={["1"]={red={{entity_id=1,circuit_id=1},{entity_id=2,circuit_id=2}}},["2"]={red={{entity_id=2,circuit_id=1}}}}}}
end

function textToSpeech.init()
	-- get word phoneme definitions, and phoneme timings from respective modules, 
	-- factorio lua doesn't include file io
	cmuDictFileTable = cmuDict.cmuTable
	timingsTable = timings.timingsTable
end

function textToSpeech.convertText(text, entityArrangementType)
	local phonemesTable = {}
	wordIndexes = {}

	phonemeCounter = 0
	-- process input text into phoneme representation
	-- and record where words end, for pauses (see next for loop)
	for word in string.gmatch(text, "[^%s]+") do
		word = string.upper( word )
		if cmuDictFileTable[word] then
			
			phonemesString = cmuDictFileTable[word]
			for v in string.gmatch(phonemesString, "[^%s]+") do
				table.insert(phonemesTable, v)
				phonemeCounter = phonemeCounter + 1
			end
		end
		table.insert( wordIndexes, phonemeCounter )
	end

	entities = {}
	timeCounter=1
	-- TODO: reduce for loops in convertText method 
	-- go through each phoneme and work out total time
	for k,v in pairs(phonemesTable) do
		if indexOf(wordIndexes, k) then
			-- if at end of word add phoneme length plus pause to timer
			timeCounter = timeCounter + ((timingsTable[indexOf(phonemesList,v)]*60)/1000) + 15
			else
				-- otherwise just increment the timer by the length of the phoneme
				timeCounter = timeCounter + (timingsTable[indexOf(phonemesList,v)]*60)/1000
		end
	end

	length = math.ceil( timeCounter )

	timerEntities = getTimerEntities(length)
	table.insert( entities,  timerEntities[1] )
	table.insert( entities,  timerEntities[2] )

	entityNum = 3
	xPos = 0.0
	yPos= 3.5
	timeCounter=1
	goRight=true
	len = #phonemesTable
	for k,v in pairs(phonemesTable) do
		
		-- print(k,v,indexOf(phonemesList,v),timingsTable[k])

		entity = {entity_number=entityNum,name="decider-combinator",position={x=xPos,y=yPos}, direction=4, 
		control_behavior={decider_conditions={first_signal={type="virtual",name="signal-0"},constant=math.ceil( timeCounter ),comparator="=",output_signal={type="virtual",name="signal-1"},copy_count_from_input=false}},
		connections={["1"]={red={{entity_id=entityNum - 1,circuit_id=1}}}
		}}
		
		table.insert(entities,entity)

		entityNum= entityNum+1

		-- xPos = xPos + 1
		xPos, yPos, goRight = textToSpeech.doEntityArrangement(entityArrangementType, k, xPos, yPos, goRight)

		-- increment timer
		if indexOf(wordIndexes, k) then
			timeCounter = timeCounter + ((timingsTable[indexOf(phonemesList,v)]*60)/1000) + 15
			else
				timeCounter = timeCounter + (timingsTable[indexOf(phonemesList,v)]*60)/1000
		end
	end
	
	-- add speaker entities
	-- usedNotesSet for creating only 1 speaker per phoneme/note
	local usedNotesSet = {}
	xPos = 0
	yPos = 5
	goRight = true
	for k,v in pairs(phonemesTable) do

		noteIndex = indexOf(phonemesList,v) - 1

		-- if speaker for note already exists don't add speaker
		if not indexOf(usedNotesSet,noteIndex) then
			
			else
				entity = {entity_number=entityNum,name="programmable-speaker",position={x=xPos,y=yPos}, direction=4, 
									control_behavior={circuit_condition={first_signal={type="virtual",name="signal-1"},constant=indexOf(phonemesList,v),comparator="="},
										circuit_parameters = {signal_value_is_pitch = false, instrument_id = 12, note_id = noteIndex} },
									connections={["1"]={red={{entity_id=entityNum - len,circuit_id=2}}}},
									parameters = {playback_volume=1.0,playback_globally=true,allow_polyphony=true},
									alert_parameters = {show_alert = false, show_on_map = false, alert_message=""}}
									
				table.insert(entities,entity)

				entityNum= entityNum+1
				-- xPos = xPos + 1
				xPos, yPos, goRight = textToSpeech.doEntityArrangement(entityArrangementType, k, xPos, yPos, goRight)
		end

		-- entity = {entity_number=entityNum,name="programmable-speaker",position={x=xPos,y=yPos}, direction=4, 
		-- control_behavior={circuit_condition={first_signal={type="virtual",name="signal-1"},constant=1,comparator="="},
		-- 	circuit_parameters = {signal_value_is_pitch = false, instrument_id = 12, note_id = noteIndex} },
		-- connections={["1"]={red={{entity_id=entityNum - len,circuit_id=2}}}},
		-- parameters = {playback_volume=1.0,playback_globally=true,allow_polyphony=true},
		-- alert_parameters = {show_alert = false, show_on_map = false, alert_message=""}}
		
		-- table.insert(entities,entity)

		-- entityNum= entityNum+1
		-- -- xPos = xPos + 1
		-- xPos, yPos, goRight = textToSpeech.doEntityArrangement(entityArrangementType, k, xPos, yPos, goRight)
	end

	return entities
end

function textToSpeech.doEntityArrangement(entityArrangementType, index, x, y, goRight)
	
	if goRight==nil then
		goRight=true
	end

	if entityArrangementType==entityArrangementsList["linear"] then
		
		x = x + 1

		elseif entityArrangementType==entityArrangementsList["block"] then
			
			if goRight then
				x = x+1
			else 
				x = x-1
			end
			
			-- increment y onto new line if needed
			if (index)%16==0 then
				y = y+3
			end

			-- change direction if needed
			if x==-1 then
				goRight = true
				x = 0
				elseif x==16 then
					goRight = false
					x = 15
			end

	end
print(x,y,goRight)
return x,y,goRight

end

-- Check if the word phoneme definitions and sound timings are loaded.
-- 
function textToSpeech.are_definitions_loaded()
	if cmuDictFileTable==nil or timingsTable==nil then
		return false
		else
		return true
	end
end
return textToSpeech